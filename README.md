### 2020-12-10
Грачев, Тихонков
Цель: переписать хендлеры на SQL-билдеры (и если успеем, то подебажить /appt_sale)
Результат: переписали соединение с базой и первые 4 хэндлера с использованием SQL билдеров из peewee.
Оценка: 5

### 2020-12-03
Губкин, Грачев
Цель: реализовать /appt_sale. 
Результат: Запрос сделали, возможно, с багами, затестить не успели. 

### 2020-11-26
Волкова, Губкин
Цель: реализовать две оставшиеся функции из старого ТЗ. Начать писать новое задание
Результат: реализовали две функции \apartments \countries и сделали view для \get_price и саму \get_price (но не поняли что с min_price)
Оценка: 5

### 2020-11-19
Русанов, Волкова
Цель: Мы поздно прочитали комментарий в слаке. Но вообще цель была реализовать как можно больше функций и добавить подключение к постгресу
