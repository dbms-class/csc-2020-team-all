...
## Цель 19.11.2020: Реализовать метод update_timeline. 
...
## Занятие 26.11.2020
* **Цель**: реализовать метод `get_timetable`
* **Сеньор**: Алексей Ростовский
* **Джуниор**: Михаил Иванов
* **Результат**: частично реализовано


## Занятие 03.12.2020
* **Цель**: реализовать метод `get_timetable`
* **Сеньор**: Михаил Иванов
* **Джуниор**: Алексей Ростовский
* **Результат**: создано представление, метод реализован в парадигме ORM и протестирован

## Занятие 10.12.2020
* **Цель**: Сделать необходимые `VIEW` для метода `shift_timetable`, начать реализовывать метод
* **Сеньор**: Михаил Иванов
* **Джуниор**: Евгений Олешкевич
* **Результат**: Начали делать метод, создали простой `VIEW` для тайм-слотов, написали прямо в `project5.sql` макет для запроса тех `route_stop`, которые будет необходимо сдвинуть (к сожалению, сделать универсальный `VIEW` для этого метода не удастся, поскольку он зависит от очень многомерного списка остановок-платформ). С целью справились, всё чётенько, 5.